...
namespace level_making
{
    ...
    public static class LevelsMaker
    {
        static public void UpdateLevelValues(ref LevelContaining levelContaining, int level) {
            ...
            levelContaining.spawningPoints = (int) ((1f / Mathf.Pow (level, 0.5f) * (Mathf.Pow (levelContaining.goalPoints, 1.15f) * ((1f - (normalized_level / 2f)) / 10f)) + (10 * level)));
            levelContaining.spawningPointsSpread = (int) (levelContaining.spawningPoints / (10f - (normalized_level * 5f)));
