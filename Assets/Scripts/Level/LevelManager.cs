...
namespace LevelManaging
{
	public class LevelManager : MonoBehaviour
	{
		...
		private bool CanSpawn() {
			...
			float sum_of_enemoes_points = 0;
			foreach (var enem in Enemy.instances) sum_of_enemoes_points += enem.points;

			return Time.timeSinceLevelLoad >= next_spawn_time
				&& levelContaining.goalPoints > (levelContaining.pointsTaken + sum_of_enemoes_points);
