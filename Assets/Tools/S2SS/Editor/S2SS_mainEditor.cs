...
public class S2SS_mainEditor : EditorWindow
{
    ...
    private void OnGUI() {
        ...
        if (target.FindProperty (nameof (textures)).arraySize > 1) {
            ...
            EditorGUILayout.LabelField ($"each sequence\t: width : {textures[ 0 ].width} px\theight : {textures[ 0 ].height} px");
            EditorGUILayout.LabelField ($"result\t\t: width : {(textures[ 0 ].width + margin) * spritesInOneRow} px\theight : {(textures[ 0 ].height + margin) * Mathf.CeilToInt ((float) textures.Length / spritesInOneRow)} px");
            ...
            if (GUILayout.Button ("Generate", GUILayout.Height (30))) {
                ...
                var path = EditorUtility.SaveFilePanelInProject ("new spritesheet file", Regex.Replace (textures[ 0 ].name, @"[\d-]", string.Empty), "png", "Select where to save the spritesheet");
